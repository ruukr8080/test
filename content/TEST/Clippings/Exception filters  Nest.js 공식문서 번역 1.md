---
title: "Exception filters | Nest.js 공식문서 번역"
author:
...
Nest는 어플리케이션의 모든 처리되지 않은 예외를 처리하는 역할을 하는 **예외 층(Exceptions Layer)**을 갖고 있습니다. 어플리케이션 코드에서 예외가 처리되지 않았을 때, 해당 예외가 예외 층에서 잡혀서, 자동으로 적당한 유저 친화적 응답을 전송합니다.

![](https://doralife12.gitbook.io/~gitbook/image?url=https%3A%2F%2Fdocs.nestjs.com%2Fassets%2FFilter_1.png&width=768&dpr=4&quality=100&sign=884cdaf1&sv=1)
...
대부분의 경우, 사용자 지정 예외를 만들 필요는 없고, 다음 섹션에서 설명하는 Nest 내장 HTTP 예외를 사용하면 됩니다. 만약 사용자 지정 예외를 만들어야만 한다면, `HttpException` 클래스를 상속받은 사용자 지정 예외들이 위치할 **예외 디렉터리(Exception Hierarchy)**를 만드는 것이 좋습니다. 이렇게 하면, Nest가 예외를 인식해서 자동으로 에러에 대한 응답을 처리하게 됩니다. 아래와 같은 사용자 지정 예외를 만들어봅시다.
